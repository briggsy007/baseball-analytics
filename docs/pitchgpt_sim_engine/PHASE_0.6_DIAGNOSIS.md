...
| mean PA length (pitches) | 3.2249 | **3.6975** | 3.8858 | ±0.5 | **PASS** (Δ −0.188) [TAINTED and PERMANENTLY UNEARNED - pos-0 calibration was fit on the eval cohort; the Phase 0.6.2 re-evaluation that would have re-earned this PASS never ran because 0.6.2 was KILLED 2026-08-10 at the fit-convergence gate (claim pitchgpt_phase062_kill; docs/models/pitchgpt_phase062_results.md). Never quote as PASS for v2-era PitchGPT.] |
| calibration_valid_coverage | 1.000 | **1.000** | ≥0.95 | — | **PASS** |

**Direction of change is favorable on every gate.** K% bias halved (8.9pp → 3.8pp); BB% bias **flipped sign** (−4.5pp → +4.3pp, similar magnitude); mean PA length moved from −0.66 to −0.19 (PASS [TAINTED and PERMANENTLY UNEARNED - pos-0 calibration was fit on the eval cohort; the Phase 0.6.2 re-evaluation that would have re-earned this PASS never ran because 0.6.2 was KILLED 2026-08-10 at the fit-convergence gate (claim pitchgpt_phase062_kill; docs/models/pitchgpt_phase062_results.md). Never quote as PASS for v2-era PitchGPT.]); mean wOBA gap closed by ~30%; HR% slightly worse but inside the original CI.
